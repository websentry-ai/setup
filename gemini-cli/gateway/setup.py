...
def detect_install_state() -> str:
    ...
    are never tamper-eligible, so 'tampered' is never reported. This gateway flow
    configures gemini-cli via env vars + the shared ~/.unbound/config.json, which
    is the only on-disk artifact it writes."""
    try:
        return "persisted" if (Path.home() / ".unbound" / "config.json").exists() else "fresh"
    except Exception as e:
...
def main():
    ...
    notify_setup_complete(api_key, "gemini-cli", backend_url=args.backend_url, install_state=_install_state, serial_number=_device_id)
