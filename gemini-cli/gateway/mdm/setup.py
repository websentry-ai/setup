...
def clear_setup() -> bool:
    ...
    if not user_homes:
        ...
        c1, n1, f1 = _clear_env_var_across_users("GEMINI_API_KEY", user_homes, label="API_KEY")
        c2, n2, f2 = _clear_env_var_across_users("GOOGLE_GEMINI_BASE_URL", user_homes, label="BASE_URL")
        if c1 or c2:
...
def detect_install_state() -> Optional[str]:
    ...
    'persisted' if this device already has the Unbound gateway config this script
    manages (~/.unbound/config.json for any user), else 'fresh'. None on any
    error. Coarse by design: the artifact is shared across Unbound tools."""
    try:
        for _username, home_dir in get_all_user_homes():
            if (home_dir / ".unbound" / "config.json").exists():
                return "persisted"
        return "fresh"
    except Exception as e:
...
def main():
    ...
    # Capture install state BEFORE this run rewrites the gateway config.
    ...
    if platform.system().lower() == "windows":
        ...
        try:
            ...
            if not settings_path.exists():
                ...
                debug_print(f"Created empty managed settings at {settings_path}")

            subprocess.run(
                ["setx", "GEMINI_CLI_SYSTEM_SETTINGS_PATH", str(settings_path), "/M"],
                check=False,
                capture_output=True,
                timeout=10,
            )
            debug_print("Set GEMINI_CLI_SYSTEM_SETTINGS_PATH (Machine)")
        except Exception as e:
            debug_print(f"Failed to setup Windows managed settings: {e}")

    print("\n" + "=" * 60)
    print("Setup Complete!")
    print("=" * 60)

    notify_setup_complete(gemini_api_key, "gemini-cli", backend_url=base_url, install_state=install_state, serial_number=device_id)
