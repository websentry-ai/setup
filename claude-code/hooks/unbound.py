...
def process_pre_tool_use(event: Dict, api_key: str) -> Dict:
    ...
    # Null fingerprint: we resolved no config for a real MCP server. Dispatch the
    # diagnostic here, before any decision/approval return — the gateway can't flag
    # this (a null fingerprint yields no unknown_mcp_server hint) and the approval
    # paths return early. Cooldown-guarded, so repeated calls don't re-fire.
