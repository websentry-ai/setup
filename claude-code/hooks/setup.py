...
def _backfill_collect_session(transcript_path: Path, project_cache: Optional[Dict] = None) -> Optional[Dict]:
    """Read a transcript and return {session_id, entries, projects} for
    server-side parsing.
    ...
    `projects` is the one thing the server cannot work out for itself: it maps
    each cwd in the transcript to its git origin, which requires being on this
    machine. Without it a replayed session has no repository, however complete
    the rest of the parse is."""
    entries = []
    session_id = None
    try:
...
def run_backfill(api_key: str, backend_url: str) -> None:
    ...
    try:
        ...
        sessions: List[Dict] = []
        # Shared across every transcript: one git call per checkout per run,
        # not per session.
