...
class TestApiKeyHelperSetting(unittest.TestCase):
    ...
    def test_a_deleted_helper_does_not_make_their_setting_ours(self):
        # judged on the value, so an already-missing script cannot widen the match
        for mod in MDM:
            managed = Path("/Library/Application Support/ClaudeCode")
            self.assertFalse(
                mod._is_unbound_key_helper_setting("~/.claude/bedrock_key.sh", managed))
            self.assertTrue(
