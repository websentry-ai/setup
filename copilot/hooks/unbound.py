    ...
    try:
        ...
        try:
            ...
            if not isinstance(unbound_config, dict):
                unbound_config = {}
            api_key, backend_url = _resolve_credentials(unbound_config, 'UNBOUND_COPILOT_API_KEY', 'discovery_gate')
            if not api_key:
...
def _dispatch_mcp_server_scan(server_name, server_config):
    ...
    try:
        ...
        if not isinstance(unbound_config, dict):
            unbound_config = {}
        api_key, backend_url = _resolve_credentials(unbound_config, 'UNBOUND_COPILOT_API_KEY', 'mcp_server')
