...
class TestInstallHooksForUser(unittest.TestCase):
    def setUp(self):
        ...
        # in the inner write path, so run it directly under the test user.
        p = mock.patch.object(mdm, "_run_as_user", lambda _u, fn, *a, **k: fn(*a, **k))
        p.start()
...
def _patch_main_deps(homes, install_results=True, script_text=_SCRIPT):
    ...
    patches = [
        ...
        mock.patch.object(mdm, "notify_setup_complete"),
    ]
    return patches, install


class TestMainExitSemantics(unittest.TestCase):
    def _run_main(self, homes, install_results=True, script_text=_SCRIPT):
        patches, install = _patch_main_deps(homes, install_results, script_text)
        for p in patches:
            p.start()
        self.addCleanup(mock.patch.stopall)
        return mdm.main(), install

    def test_full_success_returns_true(self):
        result, _ = self._run_main([("u1", Path("/tmp/h1"))])
        self.assertTrue(result)

    def test_no_users_returns_false(self):
        result, install = self._run_main([])
    ...
    def test_download_failure_returns_false(self):
        result, install = self._run_main([("u1", Path("/tmp/h1"))], script_text=None)
        self.assertFalse(result)
        install.assert_not_called()
