...
def read_account_identity(event: Optional[Dict] = None) -> Dict:
    ...
    whose matcher enables includeUserContext. There is no on-disk account record
    (no ~/.claude.json equivalent), so org/plan/auth_mode are always None and the
    email is read off the event's injected context."""
    email = None
    try:
...
def build_llm_exchange(event: Dict, post_tool_events: List[Dict], model: Optional[str] = None) -> Optional[Dict]:
    ...
    event. Augment injects the turn's conversation into the Stop event
    (conversation.{userPrompt,agentTextResponse,agentCodeResponse[]}); the
    PostToolUse tool calls are reconstructed from the accumulated audit log."""
    messages = []
    assistant_tool_uses = []
    ...
    user_prompt = (conversation.get('userPrompt') or '').strip() or None

    agent_text = (conversation.get('agentTextResponse') or '').strip()
    agent_code = conversation.get('agentCodeResponse')
    code_parts = []
    if isinstance(agent_code, list):
        for part in agent_code:
            if isinstance(part, str) and part.strip():
                code_parts.append(part.strip())
            elif isinstance(part, dict):
                snippet = (part.get('code') or part.get('content') or '').strip()
                if snippet:
                    code_parts.append(snippet)

    response_chunks = [c for c in ([agent_text] + code_parts) if c]
    ...
    for log_entry in post_tool_events:
        ...
        assistant_tool_uses.append({
            ...
            'file_changes': ev.get('file_changes'),
...
def send_to_api(exchange: Dict, api_key: str) -> bool:
    """Send exchange data to the Unbound audit endpoint.

    Phase 2 will add /v1/hooks/augment server-side. Until then this returns 404
    (curl -f -> rc != 0 -> False), which is a no-op — Stop never blocks."""
    if not api_key:
...
def process_stop_event(event: Dict, api_key: str):
    ...
    for log in logs:
        ...
        if name == 'SessionStart':
            session_events = []  # latest turn starts here
