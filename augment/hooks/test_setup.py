...
class TestBackfillCutoffCache(unittest.TestCase):
    ...
    def test_state_path_is_under_augment(self):
        """Backfill state lives under ~/.augment/hooks, not ~/.claude."""
        import setup
        p = setup._backfill_state_path(self.home)
        self.assertIn(".augment", str(p))
    ...
    def test_iter_transcripts_respects_cutoff(self):
        ...
        import setup
        root = self.home / ".augment" / "projects"
