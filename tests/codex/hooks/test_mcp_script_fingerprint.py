"""Local-script fingerprinting for codex/hooks/unbound.py.

Drives the real chain the PreToolUse path uses: ~/.codex/config.toml ->
...
class TestLocalScriptFingerprint(unittest.TestCase):
    ...
    def _fingerprint(self, server_config):
        # TOML literal strings: the fixtures carry Windows paths, which a basic
        # string would read as escapes.
        lines = ["[mcp_servers.ctx]", "command = '%s'" % server_config["command"]]
        args = ", ".join("'%s'" % a for a in server_config.get("args", []))
