"""The gateway-mode installer must carry --frontend-url into ~/.unbound/config.json.

The device-deployment page appends --frontend-url for every mode, so a
gateway-mode custom-tenant device that dropped it left frontend_url out of the
config the runtime reads. These cover both halves of the fix: the writer
persists frontend_url, and main() parses the flag and forwards it.
"""
...
gw = tool_module("claude-code/gateway/mdm", "setup")
...
class TestWriterPersistsFrontendUrl(unittest.TestCase):
    ...
    def test_absent_frontend_url_leaves_no_key(self):
        cfg = self._write(gateway_url="https://gw.example.com")
...
class TestMainParsesAndPersistsFrontendUrl(unittest.TestCase):
    ...
    def test_flag_lands_in_config(self):
        ...
        with mock.patch.object(gw.sys, "argv", argv), \
             ...
             mock.patch.object(gw, "fetch_api_key_from_mdm", return_value="cc-key"), \
             ...
             mock.patch.object(gw, "remove_user_level_gateway_for_user"), \
             mock.patch.object(gw, "setup_managed_settings", return_value=True), \
