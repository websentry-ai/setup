...
class TestIncidentReporting(RepoGateCase):
    ...
    def test_long_fields_are_capped_so_the_body_fits_the_pipe(self):
        ...
        self.set_policies([ORG_POLICY])
        # A resolvable path, then the padding as a separate argument: a single
        # 99999-character filename exceeds NAME_MAX, so it resolves to no
        # repository and the gate never fires, which would test nothing.
        self.bash('rm %s/notes.txt %s' % (self.out_scope, 'x' * 99999))
