...
With the org setting on, every block ends with "Enforced by Unbound · Trace ID
<id>" -- after the additionalContext Copilot folds into the reason; with it
off, output is unchanged."""
...
class AttributionCase(unittest.TestCase):
    ...
    def reason(self, response):
        """The user-visible text with the model context Copilot splices in removed,
        after checking both copies agree and nothing trails the footer."""
        shown = response['permissionDecisionReason']
        self.assertEqual(shown, response['hookSpecificOutput']['permissionDecisionReason'])
        context = response['hookSpecificOutput']['additionalContext']
        self.assertNotIn('Enforced by Unbound', context)
        self.assertEqual(shown.count(' ' + context), 1)
        return shown.replace(' ' + context, '')
...
class TestGatewayFooterOrdering(AttributionCase):
    def test_footer_moves_after_additional_context(self):
        response = self.bash(gateway={
            'decision': 'deny', 'reason': 'Blocked by policy X.' + FOOTER,
            'additionalContext': 'Do not retry.', 'unbound_trace_id': TRACE_ID,
        })
        self.assertEqual(response['permissionDecisionReason'],
                         'Blocked by policy X. Do not retry.' + FOOTER)

    def test_ask_footer_moves_after_additional_context(self):
        response = self.bash(gateway={
            'decision': 'ask', 'reason': 'Confirm policy X.' + FOOTER + '\n',
            'additionalContext': 'Needs a human.', 'unbound_trace_id': TRACE_ID,
        })
        self.assertEqual(response['permissionDecisionReason'],
                         'Confirm policy X. Needs a human.' + FOOTER)
    ...
    def test_without_trace_id_is_unchanged(self):
        ...
        self.assertEqual(response['permissionDecisionReason'],
                         'Blocked by policy X. Do not retry.')
