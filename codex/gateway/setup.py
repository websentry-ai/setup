...
def detect_install_state() -> str:
    ...
    are never tamper-eligible, so 'tampered' is never reported. Detected via the
    openai_base_url key this script manages in ~/.codex/config.toml (the file
    itself can exist for plain Codex usage, so we check for the key)."""
    try:
        config_file = Path.home() / ".codex" / "config.toml"
        if not config_file.exists():
            return "fresh"
        with open(config_file, "r", encoding="utf-8") as f:
            content = f.read()
        return "persisted" if "openai_base_url" in content else "fresh"
    except Exception as e:
...
def main():
    ...
    notify_setup_complete(api_key, "unbound-codex", backend_url=args.backend_url, install_state=_install_state, serial_number=_device_id)
