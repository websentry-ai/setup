...
REPO_GATE_STATE_FILE = Path.home() / ".codex" / "hooks" / ".repo_gate_state.json"
...
def process_pre_tool_use(event: Dict, api_key: str) -> Dict:
    ...
    that function short-circuits for apply_patch whenever no policy covers it —
    evaluating the gate inside it would silently never fire for the file tool
    the gate exists to govern. A gate block returns here without a gateway
    round trip: only this machine can resolve a path to a git root, so the
...
# Write operations, git commands and shell writes only (WEB-5456). apply_patch
# is Codex's only write tool; it has no read tool to exclude. Conversation is
# ungated. A shell call is in scope only when it runs git or mutates the working
# tree; every other command (ls, cat, npm test) is out of scope.
...
def _repo_gate_turn_id(event: Dict) -> str:
    """Turn identity so one turn burns one grace; `turn_id`, else prompt hash."""
